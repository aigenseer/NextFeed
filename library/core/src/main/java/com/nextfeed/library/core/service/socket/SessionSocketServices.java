...
public class SessionSocketServices {
    ...
    public void sendNewParticipantToAll(Integer sessionId, Participant participant){
        String path = "/session-socket/v1/socket/session/%s/notify/participant".formatted(sessionId);
    ...
    public void sendMood(Integer sessionId, Double value){
        String path = "/session-socket/v1/socket/session/%s/notify/mood".formatted(sessionId);
        serviceUtils.getInstanceInfoByName(INSTANCE_NAME).forEach(instance -> {
            try {
    ...
    public void sendClose(Integer sessionId){
        String path = "/session-socket/v1/socket/session/%s/notify/session/close".formatted(sessionId);
        serviceUtils.getInstanceInfoByName(INSTANCE_NAME).forEach(instance -> {
            try {
    ...
    public void sendConnectionStatus(Integer sessionId, List<Participant> participants){
        String path = "/session-socket/v1/socket/session/%s/notify/participants/status".formatted(sessionId);
        serviceUtils.getInstanceInfoByName(INSTANCE_NAME).forEach(instance -> {
            try {
