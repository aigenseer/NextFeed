...
public class SharedCoreGRPCService extends SharedCoreServiceGrpc.SharedCoreServiceImplBase {
    ...
    @Override
    public void notifyServiceStatusChance(Response.Empty request, StreamObserver<Response.Empty> responseObserver) {
        responseObserver.onNext(DTOResponseUtils.createEmpty());
        responseObserver.onCompleted();
        applicationEventPublisher.publishEvent(new ServiceStatusChanceEvent(this));
    }
